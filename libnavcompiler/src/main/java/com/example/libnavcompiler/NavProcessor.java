...
public class NavProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        // 通过处理器环境上下文roundEnv分别获取 项目中标记的FragmentDestination.class
        // 和ActivityDestination.class注解。
        ...
        if (!fragmentElements.isEmpty() || !activityElements.isEmpty()){
            ...
            // 生成文件到 /app/src/main/assets目录下
            FileOutputStream fos = null;
            OutputStreamWriter writer = null;
            try {
                //filer.createResource()意思是创建源文件，我们可以指定为class文件输出的地方，
                //StandardLocation.CLASS_OUTPUT：java文件生成class文件的位置，/app/build/intermediates/javac/debug/classes/目录下
                //StandardLocation.SOURCE_OUTPUT：java文件的位置，一般在/FineJetpack/app/build/generated/source/apt/目录下
                //StandardLocation.CLASS_PATH 和 StandardLocation.SOURCE_PATH用的不多，指的了这个参数，就要指定生成文件的pkg包名了
                FileObject resource = filer.createResource(StandardLocation.CLASS_OUTPUT, "", OUTPUT_FILE_NAME);
                String resourcePath = resource.toUri().getPath();// 位置在 /app/build/intermediates/javac/debug/../classes
                messager.printMessage(Diagnostic.Kind.NOTE, "resourcePath: " +resourcePath);

                // 裁剪resourcePath到合适的目录
    ...
    private void handleDestination(Set<? extends Element> elements, Class<? extends Annotation> annotaionClazz, HashMap<String, JSONObject> destMap) {
        for (Element element :elements){
            // TypeElement是Element的一种。
            // 如果我们的注解标记在了类名上。就可以直接强转一下。使用它得到全类名
            ...
            Annotation annotation = typeElement.getAnnotation(annotaionClazz);
            ...
            if (destMap.containsKey(pageUrl)){
                messager.printMessage(Diagnostic.Kind.ERROR, "不同的页面不允许使用形同的pageUrl " + clazzName);
            }else {
